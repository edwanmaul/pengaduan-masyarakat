...
<body class="bg-[#F5F5F5] font-[Inter]">
    ...
    {{-- HEADER --}}
    <header class="bg-[#043915] py-4 px-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.dashboard') }}" class="p-1">
                <img src="{{ asset('images/left-arrow-white.png') }}" alt="Back" class="w-6 h-6">
            </a>
        ...
        <a href="{{ route('admin.reports.export.csv') }}"
           class="bg-white/10 text-white font-semibold text-sm px-4 py-2 rounded-lg hover:bg-white/20 transition">
            Export CSV
        </a>
    </header>
                ...
                <div class="flex items-start justify-between gap-3">
                    ...
                    <div>
                        ...
                        <p class="font-bold text-base">
                            {{ $item->title ?? 'Judul laporan' }}
                        </p>

                        {{-- TANGGAL DIBUAT --}}
                        <p class="text-xs text-gray-500 mt-1">
                            Dibuat: {{ optional($item->created_at)->timezone('Asia/Jakarta')->format('d M Y H:i') }}
                        </p>

                        {{-- LINK PERIKSA --}}
                        <a href="{{ route('admin.reports.show', ['report' => $item, 'from' => 'latest']) }}"
                           class="text-xs text-black underline mt-1 inline-block">
                            Periksa
                        </a>
                    </div>
